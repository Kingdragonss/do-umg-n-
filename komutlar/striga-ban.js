...
if(!kullanici) return message.channel.send(new MessageEmbed().setDescription(`${message.author}, **Bir kullanıcı etiketlemelisin.** <a:atnbanned1:808705412530176031><a:atnbanned2:808705418968170597><a:atnbanned3:808705419072503838>`).setImage('https://media.discordapp.net/attachments/787761299374211072/787857265452777502/giphy.gif?width=400&height=200').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('0x800d0d').setTimestamp()).then(x => x.delete({timeout: 5000000}));
...
message.channel.send(new MessageEmbed().setDescription(`${message.author} tarafından ${kullanici} \`${sebep}\` Sebebiyle Sunucudan Yasaklandı.`).setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor('#740303').setTimestamp())  
banlog.send(new MessageEmbed().setImage('https://media.discordapp.net/attachments/787761299374211072/787857265452777502/giphy.gif?width=400&height=200').setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor('#740303').setTimestamp().setDescription(`<a:hydrasymbol:853584916448673792> **Sunucudan Yasaklandı !**\n <a:hydrasymbol:853584916448673792> **Banlayan Yetkili:** ${message.author} (\`${message.author.id}\`)\n <a:hydrasymbol:853584916448673792> **Banlanan Üye:** ${kullanici.user.tag} (\`${kullanici.user.id}\`)\n <a:hydrasymbol:853584916448673792> **Tarih:** \`${moment(Date.now()).add(10,"hours").format("HH:mm:ss DD MMMM YYYY")}\` `));
}
